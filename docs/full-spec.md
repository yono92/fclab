...
터미널/사이버펑크 컨셉의 인터랙티브 랜딩 페이지.

```
┌──────────────────────────────────────────────────────┐
│  [매트릭스 코드 레인 배경 — 마우스 인터랙티브]           │
│  [그리드 배경 + CRT 스캔라인 오버레이]                   │
│                                                       │
│  FCLab (로고 + 플로팅 애니메이션 + 글로우)               │
...
│  │ > computing wilson_score_ci(0.95)...              ││
│  │ ✓ win_rate: 62.0%  ci_95: [48.1%, 74.4%]         ││
│  │ ✓ play_style: BUILDUP (score: 78/100)             ││
│  │ ! WARN: in_box_rate=28% (bottom 35%)              ││
│  │ → report_generated. opening dashboard...          ││
│  └──────────────────────────────────────────────────┘│
│                                                       │
│  +── STAT ──────+ +── RULE ──────+ +── TREND ─────+  │
│  │ 통계 근거     │ │ 액션 제안     │ │ 변화 추적    │  │
│  │ wilson_score  │ │ evaluateRules │ │ WMA trend   │  │
│  │ CI, z-score   │ │ 15개 룰 엔진  │ │ 시각화      │  │
│  │ hover → code  │ │ hover → code  │ │ hover→code  │  │
│  +──────────────+ +──────────────+ +─────────────+  │
│                                                       │
│  [ HOW IT WORKS ]                                    │
│  (1)닉네임입력 ===> (2)데이터수집 ===> (3)통계분석 ===> (4)리포트생성│
│                                                       │
│  +─ 9 ─+  +─ 15 ─+  +─ 5 ─+  +─ 95% ─+             │
│  │통계함수│  │분석룰│  │스타일축│  │신뢰구간│             │
│  +─────+  +─────+  +─────+  +──────+             │
...
구현 특징:
- **매트릭스 코드 레인**: Canvas 기반, 통계/코드 토큰(wilson, z-score, fetch(), async 등)이 떨어짐. 마우스 반경 200px 내 가속+밝기 증가
- **터미널 프리뷰**: 9줄 타이핑 애니메이션으로 분석 플로우 시뮬레이션
- **피처 카드**: hover 시 코드 실행 결과 타이핑 이펙트
- **시스템 상태바**: 2초 간격 tick 업데이트 (uptime, mem, cpu, ping)
- **OG 이미지**: `opengraph-image.tsx`로 동적 생성 (터미널 컨셉)
...
│  [닉네임] Lv.32                           [최근 20경기 ▼] │
│                                                       │
│  🏆공식(47)  📋감독(12)  ⚽친선(3)  🎮볼타(0)         │
│                                      ↑ disabled       │
│  ┌─── 신뢰도 배지 ──────────────────────────────────┐ │
│  │ 표본: 47경기 중 20경기 | 공식경기 | 3/15~4/01     │ │
│  │ 신뢰도: ████████░░ 충분                           │ │
│  └──────────────────────────────────────────────────┘ │
│                                                       │
│  ┌── 종합 요약 카드 ─────────────────────────────────┐│
...
### 완료 (Phase 1~3)
- [x] 닉네임 검색 → 유저 대시보드
- [x] 슈팅/패스/수비/선수 탭별 분석 (백분위 포함)
- [x] 슈팅 히트맵 (SVG 피치 + 존 분석)
- [x] 최근 매치 리스트 + 매치 상세 (양쪽 유저 비교)
- [x] 신뢰도 배지 (표본 수, 기간, 갱신 시각)
- [x] 액션 제안 (15개 규칙 기반)
- [x] 메타데이터 동기화 (크론 API 엔드포인트)
- [x] 나 vs 랭커 비교 (Cohen's d 효과 크기)
- [x] 변화 추적/트렌드 (가중이동평균 차트)
- [x] 홈페이지 (터미널 컨셉 + 매트릭스 코드 레인 + OG 이미지)
- [x] 터미널 스타일 로딩 UI (전 페이지 통일)
- [x] Supabase 분석 결과 영속화 (비동기 fire-and-forget)

### 미구현 (Phase 4+)
- 메타 대시보드 (구조만 존재, 데이터 미연동)
- 상대 공략
- 선수 추천 엔진
...
| Heatmap | 커스텀 SVG | 슈팅 위치 시각화 |
| Tabs | @base-ui/react | 탭 UI |
| Testing | Vitest | 통계 함수 TDD |
| Deploy | Vercel | |
...
### 남은 작업
1. 메타 대시보드 (`/meta`) 데이터 연동 — 랭커 메타 vs 일반 유저 분리 표시
2. `/meta/players` 선수별 상세 메타 페이지
3. 랭커 데이터 수집 크론 (hourly) 구현
4. 백분위 계산 고도화 — 현재 fake percentile → 실제 분포 기반으로 전환
5. 캐시 히트 로직 — 이미 DB에 있는 매치 데이터 재활용

### 구현 완료 항목
- [x] Next.js 프로젝트 초기화 + Supabase 스키마
- [x] Nexon API 클라이언트 + Zod 파싱 (재시도, rate limit 대응)
- [x] `src/lib/stats.ts` 통계 함수 9개 (TDD)
- [x] `src/lib/action-rules.ts` 15개 규칙 엔진
- [x] `src/lib/play-style.ts` 5축 분류
- [x] 분석 컴포넌트: TrustBadge, PercentileGauge, StatCard, ActionSuggestionCard, MatchTypeChips
- [x] 차트 컴포넌트: PlayStyleRadar, ShootingHeatmap, WinRateDonut, PassDistributionDonut, ConcededTimeHistogram, GoalTimeline
- [x] 유저 대시보드 (4탭 분석 + 종합 요약)
- [x] 매치 상세 (양쪽 비교 + 히트맵 + 골 타임라인)
- [x] 나 vs 랭커 비교 (Cohen's d)
- [x] 트렌드 추적 (WMA 차트)
- [x] 홈페이지 (터미널 컨셉)
- [x] 메타데이터 동기화 크론 엔드포인트
- [x] 분석 결과 Supabase 영속화
